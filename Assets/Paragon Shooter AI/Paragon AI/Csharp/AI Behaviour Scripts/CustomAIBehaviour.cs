    ...
    public class DynamicObject : CustomAIBehaviour {
        ...
        void MoveToDynamicObject() {
            if (movementTargetTransform) {
                ...
                if (agent.remainingDistance != Mathf.Infinity && agent.pathStatus == NavMeshPathStatus.PathComplete && agent.remainingDistance <= 0 && !agent.pathPending && !baseScript.usingDynamicObject) {
                    UseDynamicObject();
